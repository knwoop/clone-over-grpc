...
import (
	...
	"github.com/knwoop/clone-over-grpc/proto"
...
func connect(args []string, svc string, stdin io.Reader, stdout io.Writer) error {
	...
	go func() {
		...
		for {
			...
			if n > 0 {
				if err := stream.Send(&proto.Data{Data: buf[:n]}); err != nil {
...
func grpcConnect(args []string, svc string) (proto.Git_ConnectClient, error) {
	var (
		addr *url.URL
		err  error
	)
	...
	gitClient := proto.NewGitClient(conn)
